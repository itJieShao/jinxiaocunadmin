<template>
  <div class="app-container">
    ...
    <el-table
      ...
      style="width: 100%"
    >
      <el-table-column width="120" align="center" label="入库单ID">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>

      <el-table-column width="160" align="center" label="仓管员">
        <template slot-scope="scope">
          <span>{{ scope.row.created_user_name }}</span>
        </template>
      </el-table-column>

      <el-table-column width="140" align="center" label="采购商品">
        <template slot-scope="scope">
          <span>{{ scope.row.good_species }}</span>
        </template>
      </el-table-column>

      <el-table-column width="160" align="center" label="实际采购总数量">
        <template slot-scope="scope">
          <span>{{ scope.row.good_real_num }}</span>
        </template>
      </el-table-column>

      <el-table-column width="160" align="center" label="入库点货数量">
        <template slot-scope="scope">
      ...
      <el-table-column width="200" align="center" label="实际入库点货数量">
        <template slot-scope="scope">
      ...
      <el-table-column width="120" align="center" label="入库仓库">
        <template slot-scope="scope">
          <span>{{ scope.row.in_store_names }}</span>
        </template>
      </el-table-column>

      <el-table-column width="200" align="center" label="创建入库单时间">
        <template slot-scope="scope">
      ...
      <el-table-column width="200" align="center" label="校对状态">
        <template slot-scope="scope">
...
<script>
import { getList } from "@/api/basic/warehousing";
...
export default {
  ...
  methods: {
    ...
    goDetail(id) {
      this.$router.push(`/basic/warehousing_detail?id=${id}`);
    },
    getList() {
      this.loading = true;
